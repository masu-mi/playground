...
func parseProblem(r io.Reader) (h, w, a, b int) {
	const (
...
func resolve(h, w, a, b int) int {
	moduloCombiInit(_m)
	sum := 0
	distToLastBlock := h - a - 1 + b
	for i := 1; i <= w-b; i++ {
		sum = moduloAdd(
			sum,
			moduloMul(
				moduloCombi(distToLastBlock+i-1, b+i-1, _m),
				moduloCombi(a-1+w-b-i, w-b-i, _m),
				_m,
			),
			_m,
		)
...
func initGrid(h, w int, initial int) [][]int {
	g := make([][]int, h+2)
	for i := 0; i < h+2; i++ {
		g[i] = make([]int, w+2)
		if initial != 0 {
